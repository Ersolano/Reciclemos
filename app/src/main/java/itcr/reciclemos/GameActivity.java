            ...
            public void run() {
                ...
                if(controller.getAllTrash().size() == 0){
                    if(controller.getScore() < minScore) {
                        showMessage(false, icon, toolBox.STRING_MSG_DIALOG_MISSES + ". Inténtelo nuevamente.\nPuntos Obtenidos: " + controller.getScore() + "/" + controller.getMaxScore() + ".\nPuntaje total: " + controller.getFinalScore());
                    } else {
                        showMessage(false, icon, toolBox.STRING_MSG_DIALOG_FLAWLESS + controller.getScore() + "/" + controller.getMaxScore() + ".\nPuntaje total: " + controller.getFinalScore());
                    }
                }
                else {
                    ...
                    if (gameProgressBar.getProgress() >= progressRate) {
                        ...
                        showMessage(false, icon, toolBox.STRING_MSG_DIALOG_NOTIME + controller.getScore() + "/" + controller.getMaxScore() + ".\nPuntaje total: " + controller.getFinalScore());
                    }
                }
            }
        };
